...
extern void *make_call(void *fn, void *regs, void *floats, int stack_count,
                       void *stack, int is_float);
...
int call(void *f, void **args, int *flags, int count, void **out) {
  ...
  for (ii = 0; ii < count; ii++) {
    ...
    if (stack_count > MAX_STACK_COUNT) {
      *out = strdup("maximum number of stack arguments reached (" _xstr(
          MAX_STACK_COUNT) ")");
      return 1;
    }
  ...
  *out = make_call(f, integers, floats_ptr, stack_count, stack,
                   flags[count] & ARG_FLAG_FLOAT);
  return 0;
}
